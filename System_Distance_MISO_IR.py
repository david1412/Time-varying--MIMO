...
def callback_all(Q):
    ...
    for ii in range(num_source):
        ...
        #####################################Interpolation method is linear#####################################################
        interp_method = mode
        D[ii, 0, :], impulse_response[0, :] = calc_impulse_response(K, N, s, phi, Phi, 'linear', h, p)
        D[ii, 1, :], impulse_response[1, :] = calc_impulse_response(K, N, s, phi, Phi, 'spline', h, p)
        D[ii, 2, :], impulse_response[2, :] = calc_impulse_response(K, N, s, phi, Phi, 'nearestNeighbour', h, p)
        D[ii, 3, :], impulse_response[3, :] = calc_impulse_response(K, N, s, phi, Phi, 'sinc', h, p)
        #Avg_D[0, ii] = 20*np.log10(average_fwai(D[ii, 0, :], np.linspace(90, 270, num=K)))
